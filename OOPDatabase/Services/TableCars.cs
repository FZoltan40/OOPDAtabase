...
namespace OOPDatabase.Services
{
    internal class TableCars : ISqlStatements
    {
        ...
        public object AddNewRecord(object newRecord, string tableName)
        {
            ...
            string sql = $"INSERT INTO {tableName} (`brand`, `type`, `mDate`) VALUES (@brand,@type,@date)";

            MySqlCommand cmd = new MySqlCommand(sql, conn.Connection);

            var record = newRecord.GetType().GetProperties();

            cmd.Parameters.AddWithValue("@brand", record[0].GetValue(newRecord));
            cmd.Parameters.AddWithValue("@type", record[1].GetValue(newRecord));
            cmd.Parameters.AddWithValue("@date", record[2].GetValue(newRecord));
        ...
        public object UpdateRecord(int id, object updateRecord, string table)
        {
            conn.Connection.Open();

            string sql = $"UPDATE {table} SET `brand`=@brand,`type`=@type,`mDate`=@release WHERE id = @id";
            ...
            cmd.Parameters.AddWithValue("@release", record[2].GetValue(updateRecord));
